...
class BaseBinaryDataset(IterableDataset[int], ABC):
    ...
    ) -> Iterator[tuple[bytes, bytes, npt.NDArray[np.float32]]]:
        """Yields tuples of (inputs_aa, targets_3di, plddt_array)."""
        pass

    def __iter__(self) -> Iterator[int]:
        """Iterate and write binary shards."""
        ...
        with (
            ...
            for inputs_aa, targets_3di, plddt_array in self.generate_records(worker_id, num_workers):
                if len(inputs_aa) != len(targets_3di) or len(inputs_aa) != len(plddt_array):
                    logger.warning(
                        f"Length mismatch: AA={len(inputs_aa)}, 3Di={len(targets_3di)}, pLDDT={len(plddt_array)}"
                    )
                    continue
...
class FoldseekDataset(BaseBinaryDataset):
    ...
    def __init__(self, db_prefix: str, out_dir: str, prefix: str | None = None) -> None:
        """Initialize dataset."""
        if prefix is None:
    ...
    def generate_records(
        ...
        """Yield records from Foldseek database."""
        ...
        with open(aa_path, "rb") as f_aa, open(ss_path, "rb") as f_ss:
            ...
            with (
                open(aa_index_path, encoding="ascii") as f_aa_idx,
                open(ss_index_path, encoding="ascii") as f_ss_idx,
            ):
...
def write_dataset_from_foldseek(
    db_prefix: str,
    out_dir: str,
    num_workers: int = 4,
